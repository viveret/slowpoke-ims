...
public class LocalIdentityAuthenticationService : IIdentityAuthenticationService
{
    ...
    public LocalIdentityAuthenticationService(Config config)
    {
        Config = config;
    }
    ...
    public ISlowPokeIdentity CurrentIdentity => (File.Exists(CurrentIdentityPath) ? ParseIdentityModel(File.ReadAllText(CurrentIdentityPath)) : null) ?? throw new Exception("No current identity");
    ...
    public void SetTrustLevel(ISlowPokeIdentity identity, TrustLevel? level, CancellationToken cancellationToken)
    {
        ...
        switch (level)
        {
            case TrustLevel.Trusted:
            {
                var list = TrustedIdentities.ToList();
                list.Add(identity);
                TrustedIdentities = list;
            }
            break;
            case TrustLevel.KnownButUntrusted:
            {
                var list = UntrustedIdentities.ToList();
                list.Add(identity);
                UntrustedIdentities = list;
            }
            break;
            case TrustLevel.Unknown:
            case null:
    ...
    {
        return TrustedIdentities.Concat(UntrustedIdentities).Where(i => i.AuthGuid == authGuid).First();
    }
}